...
namespace std {
template <> struct hash<mc::Model::Vertex> {
  size_t operator()(mc::Model::Vertex const &vertex) const {
    size_t seed = 0;
    mc::hashCombine(seed, vertex.position, vertex.color, vertex.normal,
                    vertex.uv);
    return seed;
  }
};
...
void Model::createIndexBuffers(const std::vector<uint32_t> &indices) {
  ...
  uint32_t indexSize =
      (indexType == VK_INDEX_TYPE_UINT16) ? sizeof(uint16_t) : sizeof(uint32_t);
